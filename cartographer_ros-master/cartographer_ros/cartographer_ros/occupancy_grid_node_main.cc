...
#include "cartographer_ros_msgs/OccupancyGridQuery.h"
...
class Node {
 ...
 private:
  ...
  bool OccupancyGridHandle(cartographer_ros_msgs::OccupancyGridQuery::Request &req, cartographer_ros_msgs::OccupancyGridQuery::Response &res);
  ...
  ::ros::ServiceServer server_ GUARDED_BY(mutex_);
...
Node::Node(const double resolution, const double publish_period_sec)
    : resolution_(resolution),
      ...
      server_(node_handle_.advertiseService("get_occupancy_grid", &Node::OccupancyGridHandle, this)),
...
bool Node::OccupancyGridHandle(cartographer_ros_msgs::OccupancyGridQuery::Request &req, cartographer_ros_msgs::OccupancyGridQuery::Response &res){
  std::map<SubmapId, SubmapSlice> submap_slices_Srv;
  for (const auto& submap_msg : req.submap_list.submap) {
    const SubmapId id{submap_msg.trajectory_id, submap_msg.submap_index};
    SubmapSlice& submap_slice = submap_slices_Srv[id];
  ...
  last_timestamp = req.submap_list.header.stamp;
  last_frame_id = req.submap_list.header.frame_id;

  auto painted_slices = PaintSubmapSlices(submap_slices_Srv, resolution_);
  std::unique_ptr<nav_msgs::OccupancyGrid> msg_ptr = CreateOccupancyGridMsg(
      painted_slices, resolution_, last_frame_id, last_timestamp);
  res.map = *msg_ptr;
  return true;
}
